...
Unreleased
----------
- Alarms support multiple AND-ed conditions. Pass ``conditions`` to ``create_alarm`` / ``put_alarm`` / ``update_alarm`` — a list of :class:`~pydoover.models.Condition`, all of which must hold for the alarm to fire — built with ``Condition.threshold``, ``Condition.rate``, ``Condition.compare`` or ``Condition.time_between`` (a time-of-day window, stored in UTC). The older single-condition ``key``/``operator``/``value`` form still works and is equivalent for one condition; the two are mutually exclusive. ``update_alarm`` replaces the whole set rather than merging, so send each condition's ``id`` back — that is what preserves a rate condition's baseline when a sibling condition changes
- :class:`pydoover.models.Alarm` exposes ``conditions``, ``condition_summary`` (the set rendered as one sentence, for a list subtitle) and the read-only ``condition_state``, plus ``primary_condition``, ``is_multi_condition`` and ``has_time_window``. ``key``, ``operator`` and ``value`` still describe the primary condition and are no longer required in a payload, so an alarm parses even once the server stops sending them; an alarm stored before conditions existed reports the single condition its legacy fields describe
- Applications can declare the notifications they send. Subclass :class:`pydoover.notifications.Notifications` the same way you subclass ``Tags``, set ``notifications_cls`` on your application, and send with ``await self.notifications.low_battery.send()``. Each declaration carries a default message, severity, title and :class:`~pydoover.models.NotificationPolicy`; the event name is the attribute name, matching the tags convention. Sending this way emits the canonical topic ``dev/applications/<policy>/<app_key>/<event>`` rather than falling into the temporary ``legacy/default`` namespace that every untopiced application notification lands in today, and ``Notifications.export`` writes a ``notification_schema`` into ``doover_config.json`` alongside ``config_schema`` and ``ui_schema``. The schema is what lets the Doover site offer a per-notification opt-out instead of an all-or-nothing switch
- ``send_notification`` accepts ``event`` and ``notification_policy`` on both device and processor applications, building the same canonical topic without a declaration. Prefer a declaration — only a declared notification appears in the schema, and only a notification in the schema can be opted out of individually
- Add :class:`pydoover.models.NotificationTopic`, which builds and validates canonical topics. Alarm topics are assembled server-side from the alarm's ``topic_name`` and policy, but application topics are not: anything an app sends that does not start with ``dev/`` is filed under ``legacy/default/``, so an application has to send the full topic itself
- Add :class:`pydoover.reports.ReportConfig`, the shared config schema every report generator needs — device permissions, cron schedule, timezone and the :class:`~pydoover.reports.EmailConfig` list of email destinations. Report generators previously re-declared these per app, and a new generator that missed one shipped without it (most often the email destinations, so reports were generated but never delivered). Subclass ``ReportConfig`` and add only the app-specific elements
- Fix a pulse counter silently dropping the first pulse on a digital input. The firmware measures ``dt_secs`` as the gap since the previous edge on that channel, so it has nothing to measure on the first edge (or after a dropped transition) and reports ``0.0``; the stream reader tested ``dt_secs > 0`` and discarded it. The test exists to skip the handshake message the stream opens with, which carries no ``dt_secs`` at all, so it now checks field presence instead. Applications where the pulse *count* is the payload — a vending terminal pulsing a product number, say — were not losing a tick but selling the wrong product
...
- Notification subscriptions can now exclude topics: ``topic_filter_exclude`` on :class:`~pydoover.models.NotificationSubscription` and both subscription clients. Entries are always regular expressions, in either filter mode, and suppress delivery even when ``topic_filter`` matches. Because filters are ORed with no negation, muting one topic previously meant replacing a broad filter with per-topic literals — which stopped the subscriber ever receiving a topic added later. An exclusion leaves the broad filter intact. ``create_notification_subscription`` replaces the whole subscription, so omitting the argument there clears any stored exclusions; on ``update_notification_subscription`` omitting it leaves them alone and passing an empty list clears them
