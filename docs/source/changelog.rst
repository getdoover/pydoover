...
- :class:`pydoover.reports.Application` now defaults its ``config_cls`` to ``ReportConfig``, so a report generator that declares no config of its own still gets the schedule, timezone and email fields the base class reads
